...
void cetakCatatan(int hari, int bulan, int panjangTanggal);
...
void cetakBulan(int tahun, int bulan) {
    ...
    while (hari <= hariDalamBulan) {
        while (hariDalamSeminggu < 7 && hari <= hariDalamBulan) {
            // Menghitung panjang teks yang akan dicetak
            int panjangTeks = printf("%4d", hari);

            // Cetak catatan untuk hari
            cetakCatatan(hari, bulan, panjangTeks);

            // Menambahkan spasi jika catatan tidak ada
            if (cetakCatatan(hari, bulan, 0) == 0) {
                printf(" ");
            }
