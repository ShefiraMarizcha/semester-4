...
void cetakKalender(int tahun, int bulan) {
    int hari, hariDalamMinggu = 0, hariPertamaMinggu;
...
int main() {
    ...
    int tahun, bulan, hari, jumlahHariBulan;
    char pilihan;
    char teks[100];
    ...
    do {
        ...
        int jumlahHariBulan = ((tahun % 4 == 0 && tahun % 100 != 0) || tahun % 400 == 0) ? 29 : 28;
        ...
        do {
            ...
            if (hari < 1 || hari > jumlahHariBulan) {
                printf("Tanggal tidak valid. Harap masukkan nilai antara 1 dan %d.\n", jumlahHariBulan);
            }
        } while (hari < 1 || hari > jumlahHariBulan);

        printf("\nApakah Anda ingin menambahkan catatan untuk tanggal %d %s? (Y/N): ", hari, bulanNama[bulan - 1]);
        scanf(" %c", &pilihan);
